    ...
    <div className="p-6 space-y-6">
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
        />
      </div>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map(student => {
          ...
          return (
            <div key={student.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                <div className="text-center p-2 bg-gray-50 rounded-xl">
                  <div className="text-base font-bold text-gray-700">{submissions.length}</div>
                  <div className="text-xs text-gray-400">Submitted</div>
                </div>
                <div className="text-center p-2 bg-gray-50 rounded-xl">
                  <div className={`text-base font-bold ${avgGrade !== null ? (avgGrade >= 75 ? 'text-green-600' : avgGrade >= 50 ? 'text-yellow-600' : 'text-red-600') : 'text-gray-300'}`}>
                  ...
                  {submissions.slice(0, 2).map(sub => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex items-center justify-between text-xs px-2.5 py-2 rounded-lg hover:bg-gray-50 transition-colors"
                      >
                        <span className="text-gray-600 truncate">{assign?.title ?? sub.assignmentId}</span>
                        {sub.grade !== undefined ? (
